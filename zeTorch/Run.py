...
class Run(object):
    ...
    def calibrate_data(self, calibration_file):
        ...
        min_lam = max(cal_data[:,0].min(), np.min(lam))
        max_lam = min(np.max(cal_data[:,0]), np.max(lam))
        ...
        calibrated_data = np.dstack((slashed_lam, cal_interp(slashed_lam) * slashed_counts ))[0]
